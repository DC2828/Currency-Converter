...
This project is a currency converter that applied 2 APIs for converting a currency to other currencies:
	...
	**Because of the free plan our group is using, both of the APIs will have limited request quotas every month, which means there are a limited quota every month of using this program.
		...
		The 2nd API provides 100 requests esch month:(
...
Test Case:
	Currency and Amount input page:
	(click the arrow at the right side of the box to spread the currency list)
	(input can be directly typed into the box and choices will be filtered according to the input for easier to select the right currency in the list)
		Valid test cases:
			HKD,1000,USD
			JPY,3408,GBP
		Invalid test cases:
